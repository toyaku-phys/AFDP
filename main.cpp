...
int main(int argc, char* argv[])
{
   ...
   for(int sl=0;sl<num_sampling_loop;++sl)
   {
      ...
      for(int step=begin_step; ;step+=delta_step)
      {
         ...
         if(particles.empty()){f=false;break;}
         if(sum_sum2.empty()){sum_sum2.resize(sampling_points.size());}
      ...
      ofs<<sl<<" "<<(std::get<0>(s2_stock)).get_av()<<std::endl;
   }
   ofs.close();
...
{  //Rerative Number of
   const auto is_on_range =
      ...
      {
         return (angle(center,p)<=dtheta) && (center-p).norm()<Radius_of_Sphere;
      };
   int res=0;
   for(size_t i=0,i_size=ps.size();i<i_size;++i)
   {
   ...
   const auto result =  (4*M_PI*Radius_of_Sphere*Radius_of_Sphere*res)/(Np*dS(dtheta,Radius_of_Sphere));
   return result;
}
